...
class Select extends React.Component {
	...
	componentWillMount() {
		...
		this._closeMenuIfClickedOutside = (event) => {
			...
			var eventOccuredOutsideControl = this.clickedOutsideElement(
				controlElem,
				event
			);
	...
	getStateFromValue(value, options, placeholder) {
		...
		if (!this.props.multi && values.length) {
			...
			valueForState = values
				.map((v) => {
					return v[this.props.valueKey];
				})
		...
		return {
			...
			placeholder:
				!this.props.multi && values.length
					? values[0][this.props.labelKey]
					: placeholder,
	...
	removeValue(valueToRemove) {
		this.setValue(
			this.state.values.filter(function (value) {
	...
	_getNewFocusedOption(filteredOptions) {
		for (var key in filteredOptions) {
			if (
				filteredOptions.hasOwnProperty(key) &&
				filteredOptions[key] === this.state.focusedOption
			) {
		...
		if (this.props.cacheAsyncResults) {
			for (var i = 0; i <= input.length; i++) {
				...
				if (
					...
					(input === cacheKey || this._optionsCache[cacheKey].complete)
				) {
	...
	filterOptions(options, values) {
		...
		var exclude = (values || this.state.values).map(
			(i) => i[this.props.valueKey]
		);
	...
	selectFocusedOption() {
		if (this.props.allowCreate && !this.state.focusedOption) {
			var newOption = this.props.newOptionCreator
	...
	buildMenu() {
		var focusedValue = this.state.focusedOption
			? this.state.focusedOption[this.props.valueKey]
			: null;
		var renderLabel = this.props.optionRenderer || this.renderOptionLabel;
		if (this.state.filteredOptions.length > 0) {
			focusedValue =
				focusedValue == null ? this.state.filteredOptions[0] : focusedValue;
		}
		...
		if (ops.length) {
			...
			return <div className={promptClass}>{noResultsText}</div>;
		}
	}
	...
	render() {
		...
		if (!this.state.inputValue && (!this.props.multi || !value.length)) {
			...
			if (this.props.valueRenderer && !!this.state.values.length) {
				...
				var singleValueComponent = React.createElement(
					this.props.singleValueComponent,
					{
						key: "placeholder",
						value: val,
